...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_declaration'])) {
    ...
    // Insert into database (shift_id is null for visitors)
    $stmt = $conn->prepare("INSERT INTO item_declarations (user_id, fullname, shift_id, declaration_date, time_in, items_json) VALUES (?, ?, NULL, ?, ?, ?)");
    $stmt->bind_param("sssss", $user_id, $fullname, $declaration_date, $time_now, $items_json);
...
<head>
    ...
    <title>Visitor Entry & Item Declaration - John Hay Hotels</title>
...
<body class="flex items-center justify-center min-h-screen p-4 text-slate-800">
    ...
    <div class="bg-white rounded-[2.5rem] w-full max-w-2xl shadow-2xl overflow-hidden">
        ...
        <div class="p-8 border-b border-slate-50 flex justify-between items-center bg-emerald-600 text-white">
            <div>
                <h3 class="text-2xl font-black">Visitor Entry Declaration</h3>
                <p class="text-sm font-medium text-emerald-100">Current Local Time: <?php echo date('h:i A'); ?></p>
            </div>
            <a href="../index.php" class="text-white hover:text-emerald-100 flex items-center gap-2 font-bold text-sm">
        ...
        <form action="" method="POST" class="flex flex-col">
            <div class="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="fullname" required placeholder="Enter your full name" class="w-full bg-slate-50 border border-slate-100 rounded-2xl p-4 text-sm font-bold outline-none focus:ring-2 focus:ring-emerald-500">
                </div>

                <div class="bg-emerald-50 p-4 rounded-2xl border border-emerald-100">
                    <p class="text-xs font-bold text-emerald-700 leading-relaxed">
                        <i class="fas fa-info-circle mr-2"></i> Welcome to The Manor at Camp John Hay. Please declare any items you are bringing into the premises.
                    </p>
                </div>
            ...
            <div class="p-8 border-t border-slate-100 bg-white flex justify-end gap-3">
                <button type="submit" name="submit_declaration" class="w-full bg-emerald-600 text-white px-8 py-4 rounded-2xl font-black shadow-lg shadow-emerald-500/30 hover:bg-emerald-700 transition-colors">
    ...
    <script>
        ...
        Swal.fire({ 
            ...
            text: 'Your entry and item declaration have been recorded. PH Time: <?php echo date('h:i A'); ?>', 
