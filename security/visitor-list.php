...
function renderTableRows($result) {
    ...
    ?>
        <tr><td colspan="4" class="px-8 py-10 text-center text-slate-400 font-bold text-sm">No historical records found matching your search.</td></tr>
    <?php
    endif;
    return ob_get_clean();
}
...
$stmt_history = $conn->prepare("
    ...
    LIMIT ? OFFSET ?
");
$stmt_history->bind_param("sssii", $search_param, $search_param, $search_param, $limit, $offset);
$stmt_history->execute();
$history_logs = $stmt_history->get_result();

if (isset($_GET['ajax'])) {
    echo json_encode([
        'tbody' => renderTableRows($history_logs),
...
<head>
    ...
    <title>Visitor & Personnel History</title>
...
<body class="flex h-screen overflow-hidden">
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden">
        <header class="bg-white border-b border-slate-200 h-20 flex items-center justify-between px-8 shrink-0">
            <div class="flex items-center gap-6">
                <h1 class="text-lg font-bold text-slate-800">Master Entry History</h1>
                <div class="h-8 w-px bg-slate-200"></div>
                
                <div class="flex items-center gap-2 relative">
                    <i class="fas fa-search absolute left-4 text-slate-400 text-xs"></i>
                    <input type="text" id="searchInput" oninput="handleSearch()" placeholder="Search name, ID, or YYYY-MM-DD..." class="bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2 text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500 w-72 transition-all">
                </div>
            </div>
        ...
        <div class="flex-1 p-8 overflow-y-auto">
            <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col h-full min-h-[600px]">
                <div class="px-8 py-6 border-b border-slate-50 flex justify-between items-center shrink-0">
                    <h2 class="text-lg font-black text-slate-800">Complete Log Archive</h2>
                    <span id="totalBadge" class="text-xs font-bold text-slate-400 bg-slate-50 px-3 py-1 rounded-lg">Total Records: <?php echo $total_records; ?></span>
                </div>
                    ...
                    <table class="w-full text-left">
                        <thead class="sticky top-0 bg-slate-50/95 backdrop-blur-sm z-10">
                            <tr class="text-[11px] font-black text-slate-400 uppercase tracking-widest">
                                ...
                                <th class="px-8 py-4 border-b border-slate-100 text-center">Declared Items</th>
                            </tr>
                        </thead>
                        <tbody id="tableBody" class="divide-y divide-slate-50 text-sm">
                            <?php echo renderTableRows($history_logs); ?>
                        </tbody>
                    </table>
                </div>
                
                <div class="px-8 py-5 border-t border-slate-50 flex justify-end items-center bg-slate-50/30 shrink-0" id="paginationContainer">
    ...
    <script>
        ...
        async function loadData(page) {
            ...
            try {
                const response = await fetch(`visitor-list.php?ajax=1&search=${encodeURIComponent(searchVal)}&page=${page}`);
                const data = await response.json();
                
                tbody.innerHTML = data.tbody;
                document.getElementById('paginationContainer').innerHTML = data.pagination;
                document.getElementById('totalBadge').innerText = `Total Records: ${data.total}`;
